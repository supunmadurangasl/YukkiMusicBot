...
async def start_stream(
    ...
    if await is_active_chat(CallbackQuery.message.chat.id):
        ...
        final_output = await CallbackQuery.message.reply_photo(
            photo=thumb,
            caption=(
                f"<b>🏷 Name:</b>[{title[:25]}](https://www.youtube.com/watch?v={videoid}) \n<b>👨‍💻Info</b> [📥Get More Information ](https://t.me/{BOT_USERNAME}?start=info_{videoid})\n**🎧 Requested by**: {CallbackQuery.from_user.mention} \n💡<b> Track added to the queue »</b> <b>{position}!</b>"
            ),
...
async def start_stream_audio(
    ...
    if await is_active_chat(message.chat.id):
        ...
        await mystic.delete()
        cap = f"🎶<b>Audio:</b> [Given Audio Via Telegram]({link})\n⏱<b>Duration:</b> {duration_min} \n🎧<b> Requested by:</b>{message.from_user.mention}"
        final_output = await message.reply_photo(
            photo="Utils/Telegram.JPEG",
            reply_markup=InlineKeyboardMarkup(buttons),
            caption=cap,
